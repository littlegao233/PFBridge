...
try {
    ...
    if (!Directory.Exists(custom_libs_path))
        Directory.CreateDirectory(custom_libs_path); //如果未找到目录就创建目录
...
if (Directory.Exists(custom_script_path)) {
    ...
    for (var i = 0; i < FileListAll.Count; i++) {
        ...
        if (file.toLowerCase().endsWith(".js"))
            FileListJS.push(file); //添加js文件到加载列表
    }
    ...
    if (custom_script_success_count > 0)
        api.Log(custom_script_success_count + '个脚本文件加载成功！');
    if (custom_script_failed_count > 0)
        api.LogErr(custom_script_failed_count + '个脚本文件加载失败！');
}
//#endregion
