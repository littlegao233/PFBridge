using IBoxs.Core;
using PFBridgeForXiaoLz.Plugin;
using SDK;
using SDK.Core;
using API = SDK.Core.API;
using SDK.Interface;
using System;
...
namespace XiaolzCSharp
{
    ...
    public static class AppRun
    {
        ...
        public static string apprun([MarshalAs(UnmanagedType.LPStr)] string pluginkey, [MarshalAs(UnmanagedType.LPStr)] string apidata)
        {
            ...
            try
            {
                appInfo.useproaddres = Marshal.GetFunctionPointerForDelegate(XLZMain.staticRotbotAppEnable).ToInt64();//插件启用
                appInfo.friendmsaddres = Marshal.GetFunctionPointerForDelegate(XLZMain.staticRecvicePrivateMsg).ToInt64();//接收好友信息
                appInfo.groupmsaddres = Marshal.GetFunctionPointerForDelegate(XLZMain.staticRecviceGroupMesg).ToInt64();//接收群消息
                appInfo.eventmsaddres = Marshal.GetFunctionPointerForDelegate(XLZMain.staticRecviceEventCallBack).ToInt64();//事件处理
                appInfo.setproaddres = Marshal.GetFunctionPointerForDelegate(XLZMain.staticAppSetting).ToInt64();//点击插件设置
                appInfo.unitproaddres = Marshal.GetFunctionPointerForDelegate(XLZMain.staticAppUninstall).ToInt64();//插件卸载
                appInfo.banproaddres = Marshal.GetFunctionPointerForDelegate(XLZMain.staticAppDisabled).ToInt64();//插件禁用 
            }
            catch (Exception ex)
            {
        ...
        private static void SetAppInfo(AppInfo App_Info)
        {
            App_Info.appname = "PFBridgeForXiaoLz";
            App_Info.author = "littlegao233";
    ...
    //        App_Info.appname = "PFBridgeForXiaoLz";
    //        App_Info.author = "littlegao233";
    //        App_Info.describe = "PFBridgeForXiaoLz beta by gxh (github:https://github.com/littlegao233/PFBridge)";
    //        App_Info.appv = "1.0.0";
    ...
    //        PFBridgeForXiaoLz.Plugin.App.OnStartup();
    //        return 0;
    //    }
    //    #endregion
    //    #region 插件卸载		
    ...
    //    {
    //        return Marshal.PtrToStringAnsi(GetPluginDataDirectoryFunc(PInvoke.plugin_key));
    //    }
    //    #endregion
    //    #region 发送群消息
    //    public static void SendGroupMsgCall(long group, string Message)
    //    {
    //        CallGetLoginQQ().ForEach(l => SendGroupMsg(PInvoke.plugin_key, l, group, Message, false));
    //    }
    //    #endregion
    //    #region 发送好友消息
    //    public static void SendPrivateMsgCall(long targetGroup, long targetQQ, string Message)
    //    {
    //        long MessageRandom = 0;
    //        uint MessageReq = 0;
    ...
    //        OutputLog OutputLogAPI = (OutputLog)Marshal.GetDelegateForFunctionPointer(new IntPtr(json["输出日志"]), typeof(OutputLog));
    //        OutputLogFunc = OutputLogAPI;
    //        GC.KeepAlive(OutputLogFunc);
    ...
    //    #region 函数委托指针
    //    //输出日志
    //    public static OutputLog OutputLogFunc = null;
    ...
    //    // 获取插件目录
    //    public static GetPluginDataDirectoryDelegate GetPluginDataDirectoryFunc = null;
    //    [UnmanagedFunctionPointer(CallingConvention.Winapi, CharSet = CharSet.Ansi)]
    //    public delegate IntPtr GetPluginDataDirectoryDelegate(string pkey);
    //    // 获取ClientKey
